...
import { OnboardingData, ConsumableCategory } from "@/types/kaza";
...
import { DEFAULT_NOTIFICATION_PREFS } from "@/contexts/KazaContext";
...
export default function Onboarding() {
  ...
  const [isHomeNameTouched, setIsHomeNameTouched] = useState(false);
  const trialDays = 7;
  ...
  useEffect(() => {
    if (onboardingData) {
      setData(prev => ({
        ...
        homeName: onboardingData.homeName || prev.homeName || ""
      }));
    }
  }, [onboardingData]);
        ...
        return (
          ...
          >
            ...
            <motion.div variants={staggerItem} className="space-y-4">
              <div>
                <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-2 block">{l.yourName}</Label>
                <Input
                  type="text"
              ...
              <div className="pt-2 animate-in fade-in slide-in-from-top-2 duration-500">
                <Label className="text-[10px] font-black text-muted-foreground uppercase tracking-widest mb-2 block">
                  {language === "pt-BR" ? "Nome da sua Kaza" : "Name of your Kaza"}
                </Label>
                <div className="relative">
                  <Input
                    type="text"
                    placeholder={language === "pt-BR" ? "Ex: Casa da Praia" : "Ex: Beach House"}
                    value={data.homeName || ""}
                    onChange={(e) => {
                      setIsHomeNameTouched(true);
                      setData((prev) => ({ ...prev, homeName: e.target.value }));
                    }}
                    className="h-14 rounded-2xl text-lg font-medium bg-white dark:bg-white/5 border-black/[0.06] dark:border-white/10 px-5 transition-all focus:ring-2 focus:ring-primary/30 focus:border-primary"
                  />
                ...
                <p className="mt-2 text-[11px] text-muted-foreground/60 px-1 italic">
                  {language === "pt-BR" 
                    ? "Isso ajudará a identificar sua casa nos convites e alertas." 
                    : "This will help identify your home in invitations and alerts."}
                </p>
              </div>
            </motion.div>
